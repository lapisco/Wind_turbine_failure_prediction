...
SENSOR = ['SENSORC', 'SENSORV']

def run(args):
    for sensor in SENSOR:
        args['output'] = os.path.join(RESULTS_FOLDER, sensor, 'union')
        for feature_set in DATA[sensor].keys():
            ...
            PIPELINE_BASE_NAME + '_{}_{}_union.pkl'.format(sensor, feature_set))    
            ...
            for classifier in CLASSIFIERS:
                ...
                clf_name = CLASSIFIER_BASE_NAME + '_{}_{}_{}_union.pkl'.format(sensor, feature_set, classifier)
                ...
                # Dump CLF
                logging.debug('Dumping classifier...')
                clf.fit(X, y)
                joblib.dump(clf, os.path.join(CLASSIFIER_FOLDER, clf_name))
